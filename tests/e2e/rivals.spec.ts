...
test('ranks the companies, runs a campaign, buys a rival out and races another for a tender', async ({ page }, testInfo) => {
  ...
  // A campaign in Havenport: a share of the city at once, then a wait before the next.
  await havenport.locator('[data-action="run-campaign"]').click();
  await expect(credits).toHaveText('54,000 credits');
  await expect(havenport.locator('.share-bar__part[data-company-id="player"]')).toBeVisible();
  await expect(havenport.locator('.city-card__share')).toHaveText(/^Your share [1-9]\d?%$/);
  await expect(havenport.locator('.city-card__wait')).toHaveText(/^Next campaign in \d+:\d\d$/);

  // Worth more than Havenport Express: bought out, its standing is the company's, and so is the city.
  ...
  await expect(page.locator('.rival-news__item').nth(0)).toHaveText(/^Havenport: Kuzey Lojistik takes the lead/);
  await expect(page.locator('.rival-news__item').nth(1)).toHaveText(/^You bought Havenport Express out(just now|1 min ago)$/);
