...
// --- Drawing helpers: anti-aliased shapes that write the glow mask into the alpha. -----------------------------

function fillRect(image: PixelImage, rect: AtlasRect, color: Rgb, alpha: number): void {
...
function shadeRect(image: PixelImage, rect: AtlasRect, shader: (u: number, v: number, x: number, y: number, out: Paint) => void, alpha = DARK): void {
...
/** Sets `out` to `r`, `g`, `b`. */
function paint(out: Paint, r: number, g: number, b: number): void {
  out[0] = r;
  out[1] = g;
  out[2] = b;
}
...
function roundedBox(image: PixelImage, x0: number, y0: number, x1: number, y1: number, radius: number, color: Rgb, alpha: number): void {
...
function line(image: PixelImage, ax: number, ay: number, bx: number, by: number, width: number, color: Rgb, alpha: number): void {
  const half = width / 2;
  const dx = bx - ax;
  const dy = by - ay;
...
function centredText(image: PixelImage, text: string, cx: number, cy: number, height: number, color: Rgb, alpha: number, weight = 0.14): void {
