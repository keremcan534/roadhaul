...
- The weather is not saved: each session starts clear, unless Settings hold one kind of weather (a device setting). `?weather=clear|cloudy|rain|dusk|night|dawn` fixes the weather for testing.
...
| Detailed traffic | ✅ | The cars (hatchbacks, and now saloons with a boot too), minibuses and lorries around the truck are drawn from side profiles: arches over the wheels, a raked windscreen, glass between the pillars, doors' lines and handles, bumpers, grilles, plates, mirrors and rims. The lorry has its cab in the fleet's paint, a white ribbed box with the fleet's stripe, rear doors, a tank and guards, and amber side markers lit at night; the minibus's windows light up at night. About 820 triangles a car, 730 a minibus and 1,070 a lorry; one draw call per kind (the saloon one more) |
...
| Rival companies (spec §65 V3) | ✅ | **The rivals.** Three rival firms, one at home in each city: Yeniliman Express (quick box trucks), Başakova Cargo (reefers) and Demirkent Haulage (flatbeds). Their trucks take contracts between the cities, earn money, and buy more trucks (up to four or five). **Standing.** Every delivery wins its company standing in both its cities: 20 for one the player drives, 8 for a fleet or rival truck's. Standing halves every half-hour. The company with the most standing, and at least 35% of it, leads a city, and contracts from a city the player leads pay 15% more. **The war.** Aggressive rivals send trucks and campaigns into the player's cities, and every rival defends a slipping lead. The player can campaign too: 6,000 credits for 40 points, once every 10 minutes in a city. **Tenders.** From the third minute, a tender comes to the job board every 8 minutes, raced by the rival at home at either end from the moment of loading; the first to unload wins the prize (60% of the pay) and 30 points. **League and buy-outs.** The companies are ranked by value, and a rival the player's company outgrows can be bought out for 1.25 times its value; its standing becomes the player's. **Where it shows.** The Rivals page (league, cities, rivals, news), the tender first on the job board, the race on the HUD, the result's lines, cities tinted in their leader's colour and the rivals' trucks on the maps. The rivals work on while the game is closed. Save v10 |
| Rival and fleet trucks on the road | ✅ | The fleet's and the rivals' trucks near the player drive in the traffic as lorries in their colours: a fleet truck in its paint, a rival's in the company's. They come in where the maps put them, out of sight as traffic appears, and take the way to the depot their contract ends at through the lanes. There they leave the road. The maps show each one where it drives. A new game's second rival truck starts at another rival's depot, so a rival's trucks do not drive in convoy |
| The map's legend and cards | ✅ | The full map names the companies' colours in a legend. A tap on a truck's arrow says whose it is (a rival, or the fleet's driver) and where it is bound, the tender's racer included; a tap in a city says who leads it, with the companies' shares and the company's own (or its leader's bonus) |

### The player's next list (between steps 28 and 29)

The player asked how to give a hired driver a truck, reported lamps showing where they should not at night ("arkamdan gelen arabalar için… ışıklar bide nesnelerin arkasına geçiyor"), and asked for a weather choice in Settings, among other things (a global theme, seasons, a bigger map with rivers and parks, monetization, deeper progression: later rounds).
