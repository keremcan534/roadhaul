...
| SaveGameData | domain | `src/domain/save/` | Versioned save schema (v9), new-game state, migrations, validation of loaded saves | Definitions | none |
...
| Save migrations and validation | domain | `src/domain/save/saveMigrations.ts`, `validateSaveGameData.ts` | Upgrade old saves (v1 → … → v9), refuse newer ones, check every field and reference (the fleet's drivers, trucks and contracts too) | ContentCatalog | none |
| EconomyService | systems | `src/systems/economy/EconomyService.ts` | The only owner of money (spec §13, §51): pays deliveries, spends, prices fuel and repairs; the fleet's income and fees go through it too | EventBus | listens `MissionCompleted`; emits `MoneyChanged` |
...
| GameSessionService | systems | `src/systems/session/GameSessionService.ts` | New game or continue; hands each save section to its owner; the fleet catches up with the time away on continuing; autosaves | every Phase 3 service, DrivingService, MissionService, GarageService, FleetService, EventService, TutorialService | listens to the mission, purchase, truck, fleet, tutorial and state events to save |
...
| MapPainter | ui | `src/ui/map/MapPainter.ts` | Paints a sketch on a canvas through a viewport: only the roads in view, the route to the next bay, pins, city names, the fleet's trucks (amber, their drivers' initials on the full map), the truck and north; allocation-free | MapSketch, DrivingService, NavigationService, MissionService, FleetService | none |
