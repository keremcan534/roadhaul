...
| SaveGameData | domain | `src/domain/save/` | Versioned save schema (v10), new-game state, migrations, validation of loaded saves | Definitions | none |
...
| CompanyHq | ui | `src/ui/hq/CompanyHq.ts`, `jobCards.ts` | The company panel over the road (spec §26): the company, credits, map and close at the top, six tabs, one scrolling list; the job board (spec §28): open contracts first, a tender first of all (its rival and prize) and the contracts of the day first in each group and marked, with when the next ones come; the leader's bonus of a city the company leads; blocked ones say what unlocks them; a busy company is told to finish its contract first | MissionService offers, DailyContracts, RivalService, Strings | none |
| HudDock | ui | `src/ui/hud/HudDock.ts` | The road's buttons to four of the panel's pages (Jobs, Truck, Garage, Events; `DOCK_TABS`); compact during a contract | Strings | none |
...
| PauseMenu, ResultDialog | ui | `src/ui/menus/` | Pause (resume, recover, fuel truck, abandon, map, settings, main menu); the itemised result or the failure reason, with a tender won or lost and the leader's bonus, then the next job or the road | Strings | none |
...
| Save migrations and validation | domain | `src/domain/save/saveMigrations.ts`, `validateSaveGameData.ts` | Upgrade old saves (v1 → … → v10), refuse newer ones, check every field and reference (the fleet's drivers, trucks and contracts, the rivals, standing and tenders too) | ContentCatalog | none |
| EconomyService | systems | `src/systems/economy/EconomyService.ts` | The only owner of money (spec §13, §51): pays deliveries, spends, prices fuel and repairs; the fleet's income and fees, the leader's bonus, tender prizes, campaigns and buy-outs go through it too | EventBus | listens `MissionCompleted`; emits `MoneyChanged` |
...
| GameSessionService | systems | `src/systems/session/GameSessionService.ts` | New game or continue; hands each save section to its owner; the rivals, then the fleet, catch up with the time away on continuing; autosaves | every Phase 3 service, DrivingService, MissionService, GarageService, FleetService, RivalService, EventService, TutorialService | listens to the mission, purchase, truck, fleet, rivals, tutorial and state events to save |
...
| GarageService | systems | `src/systems/vehicles/GarageService.ts` | The company's trucks and the active one (spec §32 GarageState); the dealer, a model as often as the garage holds (2 to 8 trucks by level); switching in place, never to a truck out with a driver; which driver has each truck, and the fleet trucks' own wear and repairs; fits the active truck's upgrades to driving, fuel and missions; the paint shop (any owned truck, the factory colour back for free) | DrivingService, MissionService, FuelService, DamageService, EconomyService | emits `VehiclePurchased`, `ActiveVehicleChanged`, `VehiclePainted` |
...
| DriverDefinition | data | `src/data/definitions/DriverDefinition.ts`, `src/data/content/drivers.ts` | The drivers the company can hire (spec §27): stars, pace, accident risk, share of the pay, hiring fee, the company level they join from | Validator | none |
| Fleet contracts | domain | `src/domain/fleet/fleetJobs.ts` | A hired driver's next contract: from their city to another, a cargo the truck carries, the pay (the contracts' formula at the fleet's share), the driver's share, the diesel, the time on the road and whether the truck comes back damaged; seeded | contractReward, fuelUsedLiters, SeededRandom | none |
| FleetService | systems | `src/systems/fleet/FleetService.ts` | The fleet (spec §27): hires and lets go, hands drivers trucks and calls them back, runs their contracts every fixed step and pays the company, sends worn trucks to the workshop, catches up with the time the game was closed, and places each truck on the map | ContentCatalog, DepotRoads, GarageService, EconomyService, CompanyService | emits `DriverHired`, `DriverDismissed`, `FleetTruckAssigned`, `FleetJobCompleted`, `FleetTruckRepaired`, `FleetCaughtUp` |
| DepotRoads | systems | `src/systems/fleet/DepotRoads.ts` | The market the fleet and the rivals plan on: the cities with a depot on the map being driven, the road between their bays and the way it takes, each worked out once; `placeOnJob` puts a truck on its contract's way | ContentCatalog, DrivingService (the map's roads) | none |
| Fleet page | ui | `src/ui/hq/fleetPage.ts` | The company panel's Fleet page: every truck (the player's, those in the garage to send out or drive, those out with their contract, progress and a call back), the hired drivers and their records, the drivers looking for work | FleetService, GarageService (read only) | none |
| RivalCompanyDefinition | data | `src/data/definitions/RivalCompanyDefinition.ts`, `src/data/content/rivals.ts` | The region's rival haulage companies (spec §65 V3): one at home in each city, with its colour, truck model, fleet size, money, pace and aggression | Validator | none |
| Standing and the rivals' moves | domain | `src/domain/rivals/StandingBoard.ts`, `rivalMoves.ts` | Each company's standing in each city, halving every half-hour, and who leads it (a leader holds on until clearly overtaken); where a rival's truck goes and where it campaigns | none | none |
| Tenders | domain | `src/domain/rivals/tenders.ts` | A generated contract the company can take now, raced by the rival at home at either end, with a prize | contractGenerator, SeededRandom | none |
| RivalService | systems | `src/systems/rivals/RivalService.ts`, `TenderBoard.ts` | The rivals: their trucks' contracts, money, new trucks and campaigns; everyone's standing from every delivery; the leader's bonus; the company's campaigns; tenders on the job board and their races; buying a rival out; the league, the news and the rivals' trucks on the map; catching up with the time away | ContentCatalog, DepotRoads, MissionService, GarageService, EconomyService, CompanyService, Clock | listens `MissionStateChanged`, `MissionCompleted`, `MissionFailed`, `FleetJobCompleted`; emits `CityLeaderChanged`, `CampaignRun`, `RivalTruckBought`, `LeaderBonusPaid`, `TenderPosted`, `TenderRivalArrived`, `TenderDecided`, `RivalAcquired` |
| CombinedContracts | systems | `src/systems/missions/CombinedContracts.ts` | Several sources of generated contracts on one job board: the contracts of the day and the tender | ContractSource | none |
| Rivals page | ui | `src/ui/hq/rivalsPage.ts` | The company panel's Rivals page: the league by value, each city's shares in the companies' colours with its leader and the campaign button or its wait, each rival with its buy-out price, the news | RivalService, EconomyService, CompanyService (read only) | none |
...
| System | Layer | Location | Responsibility | Depends on | Events |
|---|---|---|---|---|---|
| TrafficVehicleDefinition, trafficVehicles | data | `src/data/definitions/TrafficVehicleDefinition.ts`, `src/data/content/trafficVehicles.ts` | Kinds of NPC vehicle (spec §19: car, minibus, lorry, bus; a car a hatchback or a saloon): size, share of the speed limit, pull-away, how common, paint colours | Validator | none |
...
| TrafficView | presentation | `src/presentation/traffic/TrafficView.ts` | Low-poly, detailed vehicles (under 1,100 triangles each): a hatchback, a saloon, a minibus and a box lorry drawn from side profiles (arches over the wheels, a raked windscreen, glass between the pillars, doors' lines and handles, bumpers, grilles, plates, mirrors, rims), the lorry's cab in its paint with a white ribbed box, and the city bus two-tone, with window panes, doors, a windscreen under a route sign and air-conditioning on the roof; one instanced mesh per kind, painted per vehicle (only the painted parts; glass, tyres and trim keep their colour), the buses' and minibuses' windows, the route signs and the lorries' side markers lit at night, interpolated between steps, each on a soft shadow (one more); all their lamps in one more, glowing at night; the headlamps of the vehicles nearest a point, for `LampLighting`, and every vehicle's lamps for `WetReflections`; casting the sun's shadows on high | TrafficSimulation (read only), three | none |
...
| MapPainter | ui | `src/ui/map/MapPainter.ts` | Paints a sketch on a canvas through a viewport: only the roads in view, each city tinted in its leader's colour, the route to the next bay, pins, city names, the rivals' trucks in their colours (the one racing the company larger), the fleet's trucks (amber, their drivers' initials on the full map), the truck and north; allocation-free | MapSketch, DrivingService, NavigationService, MissionService, FleetService, RivalService | none |
...
AnalyticsService, AdService, IAPService, OnlineService, multiplayer, licensed brands, second-hand market. When they come, they enter as interfaces in systems, with implementations in platform. (The fleet's AI drivers and the tenders came early, at the player's request: FleetService and RivalService.)
