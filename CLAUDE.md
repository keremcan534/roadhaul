...
Append `?debug` to the game URL for the FPS / draw-call overlay, debug logs, the T key (parks the truck in the mission's next bay) and the Y key (parks it at the rest area); `?lang=tr` or `?lang=en` picks the language; `?traffic=0` turns NPC traffic off (a number sets how many vehicles); `?weather=clear|cloudy|rain|night` fixes the weather; `?date=2026-09-30` starts the special events' calendar on that day. Desktop driving: arrows/WASD, Space brakes (hold at a standstill to reverse), C switches camera, Escape or P pauses.
